...
import { getAllRoleWorkflows, updateWorkflowRoles } from "@/api/workflow-role";
...
const RoleListModal: React.FC<Props> = ({ isOpen, onClose, workflowId }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>انتخاب نقش</DialogTitle>
        </DialogHeader>
        <Content workflowId={workflowId} onClose={onClose} />
      </DialogContent>
    </Dialog>
  );
};
...
const Content = memo(
  ({ workflowId, onClose }: { workflowId: number; onClose: () => void }) => {
    ...
    const { mutate, isPending } = useMutation({
      mutationFn: () => {
        return updateWorkflowRoles(workflowId, selectedRoles);
      },
