...
import { getAllFormEntities, updateFormEntities } from "@/api/entity";
...
const EntitiesListModal: React.FC<Props> = ({ isOpen, onClose, formId }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>انتخاب جداول</DialogTitle>
        </DialogHeader>
        <Content formId={formId} onClose={onClose} />
      </DialogContent>
    </Dialog>
  );
};
...
const Content = memo(
  ({ formId, onClose }: { formId: number; onClose: () => void }) => {
    ...
    const { data, isFetching: isLoading } = useQuery({
      queryFn: () => getAllFormEntities(formId, { size, page, search }),
      queryKey: ["form-entities", { page, size, search }]
    });
    ...
    const { mutate, isPending } = useMutation({
      mutationFn: () => {
        return updateFormEntities(formId, selectedEntities);
      },

      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: ["form-entities", formId]
        });
        onClose();
      }
    });
